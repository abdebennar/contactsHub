...
const mockAgencies: Agency[] = Array.from({ length: 50 }, (_, i) => ({
  ...
  city: ['Los Angeles', 'New York', 'Houston', 'Miami', 'Chicago', 'Philadelphia', 'Columbus'][i % 7],
...
const mockContacts: Contact[] = Array.from({ length: 200 }, (_, i) => ({
  ...
  agencyId: `agency-${(i % 50) + 1}`,
  agencyName: mockAgencies[i % 50].name
}));
...
export const mockApiClient = {
  ...
  getContacts: async (params: {
    ...
    let filtered = mockContacts.filter(c => c.agencyId === params.agencyId);
  ...
  getContactDetails: async (id: string): Promise<{ data?: ContactDetails; error?: string; limitReached?: boolean }> => {
    ...
    if (viewedContactIds.has(id)) {
      const details: ContactDetails = {
        ...contact,
        email: `${contact.firstName.toLowerCase()}.${contact.lastName.toLowerCase()}@agency.gov`,
